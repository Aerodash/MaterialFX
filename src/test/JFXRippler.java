...
public class JFXRippler extends StackPane {
	...
	public void setControl(Node control) {
		if (control != null) {
			...
			// ((Control)this.control).widthProperty().addListener((o,oldVal,newVal)
			// -> {ripplerPane.setMaxWidth((double) newVal);});
			// ((Control)this.control).heightProperty().addListener((o,oldVal,newVal)
			// -> {ripplerPane.setMaxHeight((double) newVal);});
			// }else if(this.control instanceof Pane){
			// ((Pane)this.control).widthProperty().addListener((o,oldVal,newVal)
			// -> {ripplerPane.setMaxWidth((double) newVal);});
			// ((Pane)this.control).heightProperty().addListener((o,oldVal,newVal)
			// -> {ripplerPane.setMaxHeight((double) newVal);});
			// }
	...
	protected Node getMask() {
		...
		Shape mask = new Rectangle(control.getLayoutBounds().getWidth() - 0.1 - 2 * borderWidth,
				control.getLayoutBounds().getHeight() - 0.1 - 2 * borderWidth); // -0.1
																				// to
																				// prevent
																				// resizing
																				// the
		...
		if (maskType.get().equals(JFXRippler.RipplerMask.CIRCLE)) {
			double radius = Math.min((control.getLayoutBounds().getWidth() / 2) - 0.1 - 2 * borderWidth,
					(control.getLayoutBounds().getHeight() / 2) - 0.1 - 2 * borderWidth);
			mask = new Circle(control.getLayoutBounds().getWidth() / 2, control.getLayoutBounds().getHeight() / 2,
					radius, Color.BLUE);
		}
		return mask;
	}
	...
	/**
	 * Generates ripples on the screen every 0.3 seconds or whenever the
	 * createRipple method is called. Ripples grow and fade out over 0.6 seconds
	 */
	class RippleGenerator extends Group {
				...
				if (!generating) {
					...
					ripplerPane.setOnMouseReleased((e) -> {
						...
						outAnimation.setOnFinished((event) -> {
							...
							// remove overlay rect after 200 ms in case rippler
							// is not generated
							new Thread(() -> {
								try {
									Thread.sleep(200);
		...
		public void createOverlay() {
			...
			overlayRect.setFill(new Color(((Color) ripplerFill.get()).getRed(), ((Color) ripplerFill.get()).getGreen(),
		...
		private class OverLayRipple extends Rectangle {
			...
			Timeline outAnimation = new Timeline(
					new KeyFrame(Duration.seconds(0.3), new KeyValue(opacityProperty(), 0, Interpolator.EASE_BOTH)));
			...
			public OverLayRipple() {
				...
				this.widthProperty().bind(Bindings.createDoubleBinding(() -> control.getLayoutBounds().getWidth() - 0.1,
						control.boundsInParentProperty()));
				this.heightProperty().bind(Bindings.createDoubleBinding(
						() -> control.getLayoutBounds().getHeight() - 0.1, control.boundsInParentProperty()));
				this.setOpacity(0);
			}
		}
		...
		private class Ripple extends Circle {
			...
			Timeline inAnimation = new Timeline(
					new KeyFrame(Duration.ZERO, new KeyValue(radiusProperty(), 0, Interpolator.LINEAR),
			...
			private Ripple(double centerX, double centerY) {
				...
				if (ripplerFill.get() instanceof Color) {
					Color circleColor = new Color(((Color) ripplerFill.get()).getRed(),
							((Color) ripplerFill.get()).getGreen(), ((Color) ripplerFill.get()).getBlue(), 0.3);
	...
	private StyleableObjectProperty<Paint> ripplerFill = new SimpleStyleableObjectProperty<Paint>(
			StyleableProperties.RIPPLER_FILL, JFXRippler.this, "ripplerFill", Color.rgb(0, 200, 255));
	...
	private StyleableObjectProperty<RipplerMask> maskType = new SimpleStyleableObjectProperty<RipplerMask>(
			StyleableProperties.MASK_TYPE, JFXRippler.this, "maskType", RipplerMask.RECT);
	...
	private static class StyleableProperties {
		private static final CssMetaData<JFXRippler, Paint> RIPPLER_FILL = new CssMetaData<JFXRippler, Paint>(
				"-fx-rippler-fill", PaintConverter.getInstance(), Color.rgb(0, 200, 255)) {
			@Override
		...
		private static final CssMetaData<JFXRippler, RipplerMask> MASK_TYPE = new CssMetaData<JFXRippler, RipplerMask>(
				"-fx-mask-type", MaskTypeConverter.getInstance(), RipplerMask.RECT) {
			@Override
			public boolean isSettable(JFXRippler control) {
		...
		static {
			...
			Collections.addAll(styleables, RIPPLER_FILL, MASK_TYPE);
